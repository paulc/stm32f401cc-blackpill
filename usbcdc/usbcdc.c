...
/*
 * Wait for character from USB
 */
int usb_waitc(int ms) {
	char ch;
	uint32_t rc;

	rc = xQueueReceive(usb_rxq,&ch,pdMS_TO_TICKS(ms));
	if ( rc != pdPASS )
		return -1;
	return ch;
}
 ...
 * Read upto delimeter
 */
int usb_getdelim(char delim, char *buf, int buflen) {
    char c = 0;
    memset(buf,0,buflen);
    while ((c = (char)usb_getc()) != delim) {
        if (--buflen == 0) {
            return -1;
        }
        *buf++ = c;
...
usb_start(void) {
	...
	udev = usbd_init(&stm32f207_usb_driver,&dev,&config,
		usb_strings,3,
